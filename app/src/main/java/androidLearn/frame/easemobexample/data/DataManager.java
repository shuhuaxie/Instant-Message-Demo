...
public class DataManager {
  ...
  public synchronized void onAppCreate(SharedPreferences sharedPreferences) {
    String deviceId = sharedPreferences.getString(CacheKeys.DEVICEID, null);
    if (!TextUtils.isEmpty(deviceId)) {
      mCache.put(CacheKeys.DEVICEID, deviceId);
    }
  }

  public synchronized void onMainActivityDestroy(SharedPreferences sharedPreferences) {
    SharedPreferences.Editor editor = sharedPreferences.edit();
    String deviceId = get(CacheKeys.DEVICEID);
    if (!TextUtils.isEmpty(deviceId)) {
      editor.putString(CacheKeys.DEVICEID, deviceId);
    } else {
      editor.remove(CacheKeys.DEVICEID);
    }
    editor.apply();
  }
